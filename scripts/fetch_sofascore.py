...
def fetch(url):
    ...
    try:
        ...
        print("  ERR {}: {}".format(url[-50:], str(e)[:60]))
        return None
...
def infer_style_from_data(stats, last5_events, formation, avg_possession):
    ...
    FORMATION_WEIGHTS = {
        ...
        "4-3-3-0":  {"counter":3, "reactive":2, "balanced":1},
    }
    ...
    if possession >= 62:
        score["tiki-taka"] += 5.0; score["possession"] += 4.0; score["high-line"] += 1.5
    elif possession >= 57:
        score["possession"] += 3.0; score["tiki-taka"] += 2.0; score["pressing"] += 1.0
    elif possession >= 52:
        score["possession"] += 1.5; score["balanced"] += 1.0
    elif possession <= 38:
        score["counter"] += 5.0; score["direct"] += 2.0; score["low-block"] += 2.0; score["park-the-bus"] += 1.5
    elif possession <= 44:
        score["counter"] += 3.0; score["low-block"] += 2.0; score["reactive"] += 1.5
    elif possession <= 48:
        score["counter"] += 1.5; score["balanced"] += 1.0

    if accurate_pass_pct >= 88:
        score["tiki-taka"] += 4.0; score["possession"] += 2.0
    elif accurate_pass_pct >= 83:
        score["possession"] += 2.5; score["pressing"] += 1.0
    elif accurate_pass_pct >= 78:
        score["balanced"] += 1.5; score["pressing"] += 0.5
    elif accurate_pass_pct <= 68:
        score["direct"] += 3.5; score["counter"] += 1.5; score["low-block"] += 1.0
    elif accurate_pass_pct <= 73:
        score["direct"] += 2.0; score["counter"] += 1.0

    if tackles_pg >= 25:
        score["gegenpressing"] += 5.0; score["pressing"] += 3.0
    elif tackles_pg >= 20:
        score["pressing"] += 4.0; score["gegenpressing"] += 2.0
    elif tackles_pg >= 16:
        score["pressing"] += 2.0; score["balanced"] += 1.0
    elif tackles_pg <= 10:
        score["low-block"] += 2.0; score["possession"] += 1.0; score["tiki-taka"] += 1.0

    if interceptions_pg >= 10:
        score["pressing"] += 3.0; score["gegenpressing"] += 2.0; score["high-line"] += 1.5
    elif interceptions_pg >= 7:
        score["pressing"] += 2.0; score["balanced"] += 1.0
    elif interceptions_pg <= 3:
        score["tiki-taka"] += 1.5; score["possession"] += 1.0

    if dribbles_pg >= 7:
        score["wing-play"] += 4.0; score["tiki-taka"] += 1.5; score["counter"] += 1.0
    elif dribbles_pg >= 5:
        score["wing-play"] += 2.5; score["tiki-taka"] += 1.0
    elif dribbles_pg <= 1.5:
        score["direct"] += 2.0; score["low-block"] += 1.0

    if shots_pg >= 18:
        score["pressing"] += 2.5; score["high-line"] += 2.0; score["tiki-taka"] += 1.0
    elif shots_pg >= 14:
        score["pressing"] += 1.5; score["high-line"] += 1.0
    elif shots_pg <= 7:
        score["low-block"] += 2.5; score["park-the-bus"] += 1.5; score["counter"] += 1.5
    elif shots_pg <= 10:
        score["counter"] += 1.0; score["low-block"] += 1.0

    if big_chances_pg >= 3.0:
        score["tiki-taka"] += 2.0; score["wing-play"] += 2.0; score["pressing"] += 1.0
    elif big_chances_pg >= 2.0:
        score["possession"] += 1.5; score["wing-play"] += 1.0
    elif big_chances_pg <= 0.5:
        score["low-block"] += 2.0; score["park-the-bus"] += 1.5

    if goals_pg >= 2.5:
        score["pressing"] += 2.0; score["high-line"] += 1.5; score["tiki-taka"] += 1.0
    elif goals_pg >= 1.8:
        score["pressing"] += 1.0; score["balanced"] += 1.0
    elif goals_pg <= 0.7:
        score["park-the-bus"] += 3.0; score["low-block"] += 2.0; score["counter"] += 1.5

    if conceded_pg <= 0.6:
        score["high-line"] += 1.5; score["low-block"] += 1.5; score["park-the-bus"] += 1.0
    elif conceded_pg >= 2.0:
        score["pressing"] += 1.5; score["high-line"] += 1.0
    elif conceded_pg >= 1.5:
        score["balanced"] += 0.5

    first_half_goals = sum(1 for e in (last5_events or []) if e.get("firstHalfGoals", 0) > 0)
    if first_half_goals >= 4:
        score["high-line"] += 2.5; score["pressing"] += 1.5; score["gegenpressing"] += 1.0
    elif first_half_goals >= 2:
        score["pressing"] += 1.0; score["high-line"] += 1.0
...
def main():
    ...
    for match in upcoming:
        ...
        if match_key in sofascore:
            continue
        next_data = fetch("https://www.sofascore.com/api/v1/unique-tournament/{}/season/{}/events/next/0".format(
            league_id, season_id))
        ...
        for ev in next_data["events"]:
            ...
            print("  {} vs {}".format(ev_home, ev_away))
